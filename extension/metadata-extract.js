/* Injected with page save — keep in sync with shared/metadata.js */
      ...
      for (const item of items) {
        ...
        for (const node of nodes) {
          ...
          const scholarly = types.some((t) =>
            /Article|ScholarlyArticle|NewsArticle/i.test(String(t))
          );
...
function extractPageMetadata() {
  ...
  if (!authors) {
    authors =
      metaContent(doc, ["citation_authors", "author", "dc.creator"]) || ld.authors || "";
  }

  let title =
    metaContent(doc, ["citation_title", "og:title", "dc.title"]) || ld.title || "";
  if (!title) {
  ...
  const year =
    parseYear(metaContent(doc, ["citation_year", "citation_date", "dc.date"])) ||
    ld.year ||
    "";
